...
int main(int argc, char** argv) {
    double TARGET_OUTLIER_RATIO = 0.3;
    string data_file = "D:/Data/tum_data_export.txt";

    if (argc > 1) TARGET_OUTLIER_RATIO = atof(argv[1]);
    ...
    // 根据外点率动态生成输出文件名，方便管理多组实验结果
    ostringstream oss_filename;
    oss_filename << "tum_results_ransac_outlier_" << fixed << setprecision(2) << TARGET_OUTLIER_RATIO << ".txt";
    ofstream outFile(oss_filename.str());
    ...
    cout << "Starting Ultimate RANSAC Benchmark..." << endl;
    cout << "Target Outlier Ratio: " << TARGET_OUTLIER_RATIO * 100.0 << "%" << endl;

    // --- 定义各个求解器的 Lambda 包装函数 ---
    ...
    auto solve_epnp = [](const vector<Vector3d>& p3, const vector<Vector2d>& p2, Matrix3d& R, Vector3d& t) -> bool {
        int sz = p3.size();
        epnp PnP;
        PnP.set_internal_parameters(0.0, 0.0, 1.0, 1.0); 
    ...
    while (inFile >> token) {
        ...
        vector<Vector3d> cpnp_P3D; cpnp_P3D.reserve(n_points * 2); // 预留足够空间
        vector<Vector2d> cpnp_P2D_norm; cpnp_P2D_norm.reserve(n_points * 2);
        ...
        // ======================= 人为注入 Extremal Outliers =======================
        int num_original_pts = n_points;
        int num_outliers_to_add = static_cast<int>(TARGET_OUTLIER_RATIO * num_original_pts / (1.0 - TARGET_OUTLIER_RATIO));

        if (num_outliers_to_add > 0) {
            std::mt19937 rng_outlier(frame_id); // 保证同一帧产生的极大外点一致
            std::uniform_int_distribution<int> uni_3d(0, num_original_pts - 1);
            
            // 构造强边缘外点，位于归一化坐标的 0.9 到 1.5 之间
            std::uniform_real_distribution<double> edge_dist(0.9, 1.5); 
            std::uniform_int_distribution<int> sign_dist(0, 1);

            for (int i = 0; i < num_outliers_to_add; ++i) {
                int idx3d = uni_3d(rng_outlier);
                cpnp_P3D.push_back(cpnp_P3D[idx3d]); // 挂载到真实的3D点上

                // 强制投影在极限边缘位置
                double sign_x = sign_dist(rng_outlier) ? 1.0 : -1.0;
                double sign_y = sign_dist(rng_outlier) ? 1.0 : -1.0;
                double bad_u_norm = sign_x * edge_dist(rng_outlier);
                double bad_v_norm = sign_y * edge_dist(rng_outlier);

                cpnp_P2D_norm.push_back(Vector2d(bad_u_norm, bad_v_norm));
            }
            n_points = cpnp_P3D.size(); // 更新RANSAC所需计算的总点数
        }
        // ==========================================================================
    ...
    cout << "  RANSAC params: 500 iters, 6-pt sample, 2 px threshold, Ext Outlier Ratio: " << TARGET_OUTLIER_RATIO * 100 << "%" << endl;
    cout << "==================================================================================" << endl;
